...
# Save as: /opt/expressroute-testing/scripts/express_route_monitoring.sh
...
collect_expressroute_metrics() {
    ...
    az network express-route list \
        ...
        > "$RESULTS_DIR/expressroute_east_details_$timestamp.json" 2>/dev/null
    ...
    az network route-table list \
        ...
        > "$RESULTS_DIR/route_tables_$timestamp.json" 2>/dev/null
...
analyze_bandwidth_patterns() {
    ...
    for session_dir in $(find "$RESULTS_DIR" -name "session_*" -type d -mtime -$days_back 2>/dev/null | sort); do
        if [ -f "$session_dir/bandwidth_summary.csv" ]; then
            ...
            }' "$session_dir/bandwidth_summary.csv" >> "$analysis_file" 2>/dev/null
            echo "" >> "$analysis_file"
        fi
    done
    ...
    cat > "$health_file" << EOF
{
    ...
    "route_tables": {
        "status": "$route_status",
        "tested": $(printf '%s\n' "${expected_routes[@]}" | jq -R . | jq -s . 2>/dev/null || echo '["EAST-SPOKE-1-PRV-RT", "EAST-SPOKE-1-PUB-RT", "EAST-HUB-GATEWAY-RT"]')
    },
    "connectivity_tests": {
...
create_performance_baseline() {
    ...
    for target in "${targets[@]}"; do
        ...
        if ping -c 1 -W 5 "$ip" >/dev/null 2>&1; then
            # Use a quick iperf3 test (5 seconds) - only if iperf3 server is available
            local bandwidth=0
            if command -v iperf3 &> /dev/null; then
                bandwidth=$(timeout 10 iperf3 -c "$ip" -t 5 -J 2>/dev/null | jq -r '.end.sum_received.bits_per_second // 0' 2>/dev/null || echo "0")
            fi
            local mbps=$(echo "scale=2; $bandwidth / 1000000" | bc 2>/dev/null || echo "0")
            echo "      \"bandwidth_mbps\": \"${mbps}\"" >> "$baseline_file"
        else
...
EOF
    ...
    local latest_session=$(find "$RESULTS_DIR" -name "session_*" -type d 2>/dev/null | sort | tail -1)
    ...
    if [ -n "$latest_session" ] && [ -d "$latest_session" ]; then
        ...
        if [ -f "$latest_session/latency_summary.csv" ]; then
            ...
            awk -F',' 'NR>1 {printf "            {\"target\": \"%s\", \"latency\": \"%s\", \"loss\": \"%s\"}%s\n", $2, $5, $6, (NR==lines) ? "" : ","}' \
                lines=$(wc -l < "$latest_session/latency_summary.csv" 2>/dev/null || echo "1") "$latest_session/latency_summary.csv" >> "$dashboard_data" 2>/dev/null
            echo "        ]," >> "$dashboard_data"
        fi
        ...
        if [ -f "$latest_session/bandwidth_summary.csv" ]; then
            ...
            awk -F',' 'NR>1 {printf "            {\"target\": \"%s\", \"protocol\": \"%s\", \"bandwidth_mbps\": \"%s\"}%s\n", $2, $4, $7, (NR==lines) ? "" : ","}' \
                lines=$(wc -l < "$latest_session/bandwidth_summary.csv" 2>/dev/null || echo "1") "$latest_session/bandwidth_summary.csv" >> "$dashboard_data" 2>/dev/null
            echo "        ]" >> "$dashboard_data"
        else
            echo "        \"bandwidth_results\": []" >> "$dashboard_data"
        fi
    ...
    echo "        \"uptime\": \"$(uptime -p 2>/dev/null || uptime)\"," >> "$dashboard_data"
...
check_performance_alerts() {
    ...
    if [ -f "$latest_session/latency_summary.csv" ]; then
        ...
        }' "$latest_session/latency_summary.csv" >> "$alert_file" 2>/dev/null
    ...
    if [ -f "$latest_session/bandwidth_summary.csv" ]; then
        ...
        }' "$latest_session/bandwidth_summary.csv" >> "$alert_file" 2>/dev/null
    fi
